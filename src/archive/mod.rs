...
impl ArchiveWriter {
    ...
    pub fn write_header(&mut self) -> Result<(), std::io::Error> {
        ...
        // ensure header is persisted before writing bundles
        self.writer.flush()
    }
    ...
    pub fn finalize(mut self) -> Result<(), ArchiverError> {
        // Обновляем заголовок с актуальным количеством файлов перед записью индекса
        self.index.header.file_count = self.index.entries.len() as u64;
        let updated_header_bytes = serde_json::to_vec(&self.index.header)?;
        self.writer.seek(SeekFrom::Start(MAGIC_BYTES.len() as u64))?;
        
        // Записываем только JSON часть заголовка (после magic bytes)
        let mut padded_header = updated_header_bytes.clone();
        if padded_header.len() < (HEADER_SIZE as usize - MAGIC_BYTES.len()) {
            padded_header.resize(HEADER_SIZE as usize - MAGIC_BYTES.len(), 0);
        }
        padded_header.truncate(HEADER_SIZE as usize - MAGIC_BYTES.len());
        self.writer.write_all(&padded_header)?;
        
        // Возвращаемся на текущую позицию для записи индекса
        self.writer.seek(SeekFrom::Start(self.current_offset))?;
